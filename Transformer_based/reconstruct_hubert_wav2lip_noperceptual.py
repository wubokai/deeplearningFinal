    ...
    def __getitem__(self, idx):
        while 1:
            ...
            if true_landmarks_fnames is None or len(true_landmarks_fnames) is None:
                continue
            if wrong_landmarks_fnames is None or len(wrong_landmarks_fnames) is None:
                continue
...
def train(device, model, train_data_loader, test_data_loader, optimizer,
    ...
    while global_epoch < nepochs:
        ...
        running_l1_loss, running_sync_loss = 0., 0.
        prog_bar = tqdm(enumerate(train_data_loader))
        for step, (x, gt, landmarks, mel, indv_mels) in prog_bar:
            model.train()
            optimizer.zero_grad()
            ...
            indv_mels = indv_mels.to(device)
            g = model(landmarks, x, indv_mels)  # B, 3, 5, 96, 96
            ...
            loss = (1 - hparams.syncnet_wt) * l1loss + hparams.syncnet_wt * sync_loss
            loss.backward()
            optimizer.step()
            ...
            if global_step == 1 or global_step % hparams.eval_interval == 0:
                with torch.no_grad():
                    ...
                    if average_sync_loss < 0.50:
                        hparams.set_hparam('syncnet_wt', 0.02)  # without image GAN a lesser weight is sufficient

            prog_bar.set_description(
                'Train L1: {}, Sync:{} :'.format(running_l1_loss / (step + 1), running_sync_loss / (step + 1)))
...
def eval_model(test_data_loader, global_step, device, model, checkpoint_dir):
    ...
    step = 0
    while 1:
        for x, gt, landmarks, mel, indv_mels in test_data_loader:
            step += 1
            model.eval()
            ...
            if step > eval_steps:
                ...
                print('Eval L1: {}, Sync: {}'.format(averaged_recon_loss, averaged_sync_loss))
