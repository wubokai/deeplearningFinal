...
def main():
    model1 = load_model_1(args.checkpoint_path2)
    ...
    if mel_length < frames_num:
        landmarks = landmarks_list[:mel_length, :, :]
    else:
        landmarks = []
        for i in range(mel_length):
            idx = i % frames_num
            landmarks.append(np.asarray(landmarks_list[idx]))

    landmarks = torch.FloatTensor(np.asarray(landmarks))
    landmarks = landmarks.unsqueeze(1).to(device)
    mel_chunks = mel_chunks.to(device)
    landmark_predicted = model1(mel_chunks, landmarks).squeeze()
    ...
    for i in range(landmark_predicted.shape[0]):
        ...
        for j in range(landmark_predicted.shape[1]):
            x1, y1 = int(landmarks[i][j][0] * 96), int(landmarks[i][j][1] * 96)
            x2, y2 = int(landmark_predicted[i][j][0] * 96), int(landmark_predicted[i][j][1] * 96)
            if x1 == x2 and y1 == y2:
    ...
    gen = datagen(full_frames.copy(), landmark_predicted.cpu(), mel_chunks)
    ...
    for i, (landmarks, img_batch, frames, coords, mel_audio) in enumerate(tqdm(gen,
        ...
        landmarks = torch.FloatTensor(landmarks).unsqueeze(1).to(device)
