...
def main():
    ...
    l1 = int(hubert_feature.shape[0])/2
    landmark = np.load(args.landmarks)

    l2 = landmark.shape[0]
    if l1 < l2:
        landmark = landmark[:l1]
    else:
        hubert_feature = hubert_feature[:2*l2]

    mel_chunks = []
    for lel in range(0, len(hubert_feature), 2):
        mel_chunks.append(hubert_feature[lel:lel+2])
    ...
    for i, (landmarks, img_batch, frames, coords, mel_audio) in enumerate(tqdm(gen,
        ...
        landmarks = torch.FloatTensor(np.transpose(landmarks, (0, 2, 1))).unsqueeze(1).to(device)
