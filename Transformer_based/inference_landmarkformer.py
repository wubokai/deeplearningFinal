...
def main():
    ...
    landmark_predicted = []
    flag = 0
    for mel_single in mel_chunks:
        mel_single = torch.FloatTensor(mel_single).unsqueeze(0).unsqueeze(0).to(device)
        if flag == 0:
            landmarks_list = landmarks_list.unsqueeze(0).unsqueeze(0).to(device)
        predicted = model(mel_single, landmarks_list)
        landmark_predicted.append(np.asarray(predicted.cpu().detach()))
        landmarks_list = predicted
        flag = 1
    landmark_predicted = torch.FloatTensor(landmark_predicted)
    print(landmark_predicted.squeeze().shape)
