...
parser.add_argument('--landmarks', type=str,
                    help='Filepath of video/audio file to use as raw landmark source', required=True)
...
def datagen(frames, landmarks, mel_chunks):
    ...
    for i, landmark in enumerate(landmarks):
        ...
        if len(img_batch) >= args.wav2lip_batch_size:
            img_batch, landmarks_batch, mel_batch = np.asarray(img_batch) / 255., np.asarray(landmarks_batch), np.asarray(mel_batch)
            ...
            landmarks_batch = np.reshape(landmarks_batch,
                                         (landmarks_batch.shape[0], landmarks_batch.shape[1], landmarks_batch.shape[2],
                                          1))
            landmarks_batch = landmarks_batch / 96.
...
def main():
    ...
    gen = datagen(full_frames.copy(), landmarks_input, mel_chunks)
    ...
    for i, (landmarks, img_batch, frames, coords, mel_audio) in enumerate(tqdm(gen,
        ...
        mel_audio = torch.FloatTensor(mel_audio).unsqueeze(1).to(device)
        if len(img_batch.size()) >4:
            mel_audio = mel_audio.unsqueeze(1)
        print(mel_audio.shape)
        with torch.no_grad():
            pred = model(landmarks, img_batch, mel_audio)
