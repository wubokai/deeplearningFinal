...
from models import Reconstructor_lip
import platform
...
def datagen(frames, landmarks):
    landmarks_batch, frames_batch, img_batch, coords_batch, true_landmarks_batch = [], [], [], [], []
    ...
    for i, landmark in enumerate(landmarks):  # 五帧，所以frames也要五帧五帧
        ...
        if len(img_batch) >= args.wav2lip_batch_size:
            img_batch, landmarks_batch = np.asarray(img_batch)/255., np.asarray(landmarks_batch)
            ...
            landmarks_batch = np.reshape(landmarks_batch,
                                         (landmarks_batch.shape[0], landmarks_batch.shape[1], landmarks_batch.shape[2], 1))
            yield img_batch, landmarks_batch, frames_batch, coords_batch
            img_batch, landmarks_batch, frames_batch, coords_batch = [], [], [], []
    ...
    if len(img_batch) > 0:
        ...
        for img in img_batch:
            print(img)
            cv2.imwrite('{}/{}.jpg'.format("/root/autodl-tmp/Transformer_based/images", num), img)
            num += 1
        landmarks_batch = np.reshape(landmarks_batch/96., (landmarks_batch.shape[0], landmarks_batch.shape[1], landmarks_batch.shape[2], 1))
        yield landmarks_batch, img_batch, frames_batch, coords_batch
...
def main():
    ...
    landmarks_input = np.load(args.landmarks)
    landmarks_length = len(landmarks_input)

    if landmarks_length < len(full_frames):
        full_frames = full_frames[:landmarks_length]
    else:
        full_frames = full_frames

    batch_size = args.wav2lip_batch_size
    gen = datagen(full_frames.copy(), landmarks_input)

    for i, (landmarks, img_batch, frames, coords) in enumerate(tqdm(gen,
                                                                    total=int(
                                                                        np.ceil(float(
                                                                            len(landmarks_input)) / batch_size)))):
        if i == 0:
            model = load_model(args.checkpoint_path)
            print("Model loaded")

            frame_h, frame_w = full_frames[0].shape[:-1]
            out = cv2.VideoWriter('temp/result001.avi',
        ...
        print(landmarks)
        with torch.no_grad():
            pred = model(landmarks, img_batch)
    ...
    out.release()
    command = 'ffmpeg -y -i {} -q:v 1 {}'.format('temp/result001.avi', args.outfile)
