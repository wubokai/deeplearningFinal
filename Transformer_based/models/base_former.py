...
class lip_former(nn.Module):
    def __init__(self):
        ...
        self.Positional_Encoding = PositionalEncoder(d_model=512)
    ...
    def forward(self, audio, landmarks):  # landmarks->(B,5,856), audio->(B, 5, 80, 16)
        ...
        audio_feature = self.layer_norm1(audio_feature)
        audio_feature = self.Positional_Encoding(audio_feature)
        ...
        landmarks_embedding = self.Positional_Encoding(landmarks_embedding)
