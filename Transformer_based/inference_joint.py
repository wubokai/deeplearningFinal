...
def main():
    ...
    print("wjpwjpwj", landmarks_list.shape)
    batch_size = args.wav2lip_batch_size
    gen = datagen(full_frames.copy(), landmarks_list.cpu(), mel_chunks)
    ...
    for i, (landmarks, img_batch, frames, coords, mel_audio) in enumerate(tqdm(gen,
        ...
        landmarks = torch.FloatTensor(np.transpose(landmarks, (0, 1, 2))).unsqueeze(1).to(device)
        ...
        with torch.no_grad():
            pred, landmarks = model1(mel_audio, landmarks, img_batch)
        ...
        for i in range(landmarks.shape[0]-1):
            ...
            for j in range(landmarks.shape[1]):
                x1, y1 = int(landmarks_list[i][j][0] * 256), int(landmarks_list[i][j][1] * 256)
                x2, y2 = int(landmarks[i][j][0] * 256), int(landmarks[i][j][1] * 256)
                print(x2, y2)
                # if x1 == x2 and y1 == y2:
            ...
            cv2.imwrite("result_joint_image/{}.jpg".format(str(i)), image)
    ...
    out.release()
    command = 'ffmpeg -y -i {} -i {} -strict -2 -q:v 1 -loglevel quiet {}'.format(args.audio, 'temp/result.avi', args.outfile)
